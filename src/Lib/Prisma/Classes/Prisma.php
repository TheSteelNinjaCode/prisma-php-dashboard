...
final class DatabaseConnection
{
    ...
    {
        $envPath = DOCUMENT_PATH . '/.env';

        if (file_exists($envPath)) {
            Dotenv::createImmutable(DOCUMENT_PATH)->safeLoad();
        }

        $databaseUrl = Env::get('DATABASE_URL') ?: null;

        if (!$databaseUrl) {
            throw new Exception(
                'DATABASE_URL is not set. On Railway set it in Variables, or add it to your local .env.'
            );
        }
    }
    ...
    private function initializePDO(): void
    {
        if ($this->_pdo === null) {
            ...
            if ($dbProvider === 'file' || $dbProvider === 'sqlite') {
                ...
                $prismaDirectory = DOCUMENT_PATH . DIRECTORY_SEPARATOR;
                ...
                $collation = $queryParams['collation'] ?? 'utf8mb4_unicode_ci';
            ...
            try {
                ...
                if ($dbProvider === 'mysql' && isset($charset) && isset($collation)) {
                    $this->_pdo->exec("SET NAMES $charset COLLATE $collation");
                }
