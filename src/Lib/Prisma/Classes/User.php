...
class User implements IModel
{
    ...
    public function __construct(PDO $pdo)
    {
        ...
        $this->_tableFieldsOnly = ['id', 'name', 'email', 'password', 'emailVerified', 'image', 'createdAt', 'updatedAt', 'roleId'];
        ...
        $this->_primaryKeyAndUniqueFields = ['id', 'email'];
        $this->_fieldsCombined = ['id', 'name', 'email', 'password', 'emailVerified', 'image', 'createdAt', 'updatedAt', 'roleId', 'userRole'];
     ...
     *
     * This method allows dynamic access to properties of the User class.
    ...
    public function __get(string $name)
    {
        ...
        throw new Exception("Property '$name' does not exist in User.");
    }
    ...
    /**
     * Performs an aggregate operation on the 'User' table. This method allows for flexible aggregation queries
     ...
     *
     * @return UserData Returns an object containing the results of the aggregate operation. The result will include keys corresponding to the specified
     *
     ...
     *     'where' => ['status' => 'active'],
     * ];
     * $result = $prisma->user->aggregate($criteria);
     ...
     * queries with conditions (WHERE clause), ordering (ORDER BY), and pagination (LIMIT and OFFSET) on the 'User' table.
     ...
     * It is capable of handling related records through the dynamically defined relations in the User model.
     ...
     *
     * @return UserData The newly created UserData record.
     *
     * @throws:
    ...
    /**
     * Bulk Create User – Fast, Safe, and Flexible
     *
     * Effortlessly add multiple User records to your database in a single, atomic operation.
     * This method is perfect for onboarding, migrations, or any scenario where you need to insert many User records at once.
     *
     * Key Features:
     * - Accepts an array of User records—add as many as you need in one call.
     * - Runs inside a transaction: if any record fails, nothing is saved.
     * - Validates required fields for every User before saving.
    ...
    /**
     * Bulk Create User and Return Created Records
     *
     * This method allows you to create multiple User records in a single operation and returns the created records.
     * It supports skipping duplicates and allows for selective field retrieval.
     *
     * @param array $data An associative array containing:
     * - 'data': A list of associative arrays representing the User records to create.
     ...
     *
     * @return array An array of created UserData objects.
     *
     ...
     * @return UserData|null The UserData record matching the criteria.
     * 
    ...
    /**
     * Retrieves a unique User record based on specified criteria.
     *
     * This method is a wrapper around `findUnique` that throws an exception if no record is found.
     * It is useful when you expect exactly one User to match the criteria and want to handle the case where none exists.
     *
     * @param array $criteria Criteria to find the unique User, same as in `findUnique`.
     * @return UserData The UserData record matching the criteria.
     * @throws Exception If no User record matches the criteria.
     */
     ...
     * @return UserData[] An array of UserData objects or an empty array
     * 
     * @example
     * Retrieve User with cursor-based pagination:
    ...
    /**
     * Retrieves the first User record that matches specified filter criteria or throws an exception if not found.
     *
     * This method is similar to `findFirst`, but it guarantees that a User record is returned. If no record
     ...
     * 
     * Returns:
     * @return UserData|array|null The updated User record or an empty array if no match is found.
     * 
     * Example Usage:
     * Example 1: Update a User's email and only return their 'id' and 'email' in the response
     * $result = $prisma->user->update([
     *   'where' => ['id' => 'someUserId'],
    ...
    /**
     * Bulk update User records with flexible conditions and smart validation.
     *
     * This method lets you update multiple User records at once, based on any conditions you specify.
     ...
     *
     * This is perfect for mass updates—like activating User, anonymizing data, or fixing typos—quickly and safely.
     */
    ...
    /**
     * Updates multiple User records and returns the updated records.
     *
     * This method allows you to update multiple User records based on specified conditions
     ...
     *
     * @return array An array of updated UserData objects.
     *
     ...
     * @return UserData|array On successful deletion, returns the deleted UserData record.
     *               If the deletion is unsuccessful due to a non-existent UserData or non-unique criteria, returns an
     ...
     * 
     * @return UserData Returns the created or updated User record.
     * 
     * Exceptions:
    ...
    /**
     * Groups records in the 'User' table based on specified criteria.
     * 
     * This function groups records in the 'User' table according to the fields specified 
    ...
    /**
     * Dynamically deletes multiple User records based on flexible criteria.
     *
     * This method enables bulk deletion of User records by specifying any combination of conditions in the 'where' key.
     * - Accepts any valid User field(s) as filter criteria.
     ...
     * @return int|array|UserData The number of records that match the criteria or detailed count per field.
     * @throws Exception Throws an exception if the database operation fails.
     *
     * Example Usage:
     * $prisma = new Prisma();
     * $countCriteria = ['status' => 'active'];
     * $result = $prisma->user->count($countCriteria);
     */
