...
class Button extends PHPX
{
    ...
    /** @property ?string $size = default|sm|lg|icon|icon-sm|icon-lg */
    ...
    private function getComputedClasses(): string
    {
        ...
        $variantKey = $this->props['variant'] ?? 'default';
        $sizeKey = $this->props['size'] ?? 'default';

        return $this->getMergeClasses(
            $baseClasses,
            $variantClasses[$variantKey] ?? $variantClasses['default'],
            $sizeClasses[$sizeKey] ?? $sizeClasses['default'],
            $this->class
        );
    }
